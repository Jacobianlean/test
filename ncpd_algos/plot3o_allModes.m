...
% This function is to plot the median convergence curve for synthetic experiments
%% Input handling
...
for i = 1:min(numAlgos, 2)
    if i == 1
        mean_colors{1} = [0.5, 0.0, 0.8];    
        all_colors{1}  = [0.8, 0.7, 1.0];    
        lineStyles{1} = '-';
    else
        mean_colors{2} = [0.92, 0.47, 0.20]; 
        all_colors{2}  = [1.0, 0.8, 0.6];    
        lineStyles{2} = '-';
    end
end
...
gap = 0.02; 
left_main = 0.10; 
right_main = left_main + 0.38 + gap; %

ax1 = axes('Position', [left_main,  0.58, 0.38, 0.35]); %  - f vs iteration
ax2 = axes('Position', [right_main, 0.58, 0.38, 0.35]); %  - f vs time
ax3 = axes('Position', [left_main,  0.12, 0.38, 0.35]); %  - e vs iteration
ax4 = axes('Position', [right_main, 0.12, 0.38, 0.35]); %  - e vs time
